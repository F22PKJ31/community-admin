			...
			<el-table :data="tableData" @selection-change="handleSelectionChange" border class="table"
				...
				<el-table-column label="图片" prop="imgUrl">
					<template slot-scope="scope">
						<img :src="scope.row.imgUrl" height="240px" width="400px"/>
					</template>
				</el-table-column>
		...
		<el-dialog :visible.sync="editVisible" title="编辑" width="30%">
			<el-form :model="form" label-width="100px" ref="form">
				...
				<el-form-item label="对应新闻">
					<el-select v-model="form.newsId">
						<el-option :key="c.newsId" :label="c.title" :value="c.newsId" v-for="c in newsData">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="图片" prop="imgUrl">
					<el-upload
						...
						<img :src="form.imgUrl" class="avatar" height="120px" v-if="form.imgUrl" width="200px">
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                this.axiosProxy.headImgList().then(response => {
                    ...
                    for (let i = 0; i < this.tableData.length; i++) {
                        this.tableData[i].title = this.newsData.find(function (x) {
                            return x.newsId === this.tableData[i].newsId;
                        }, this).title;
                    }
                })
            },
            getNewsData() {
                ...
                this.axiosProxy.getAllNewsList(queryParams).then(response => {
